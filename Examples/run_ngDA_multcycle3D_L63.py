...
period_obs = 50                             # Observation period, in number of time steps
...
def one_run(jj):
    ...
    for iM in range(n_meths-1):
        ...
        if da_method == '3DVAR':
            ...
            descriptor = 'mode'         # Descriptor to optimize, either 'mode', 'mean', or 'median'
            if descriptor == 'mode':
                l_SV, l_obs = 1.0, 1.0
...
if __name__ == '__main__':
    ...
    for w in np.arange(6,18,2):
        for s_cutoff in np.arange(0.8,2.4,0.2):

            ml_file_str =  'w'+str(w)+'_s'+str(np.round(s_cutoff,2))
            ml_file = './data/kNN_l63_'+ml_file_str+'.pkl'    # Location of the trained ML model

            n_meths = 7
            RMSE_A = np.empty((n_runs,n_meths))
            RMSE_B = np.empty((n_runs,n_meths))
            MAE_A = np.empty((n_runs,n_meths))
            MAE_B = np.empty((n_runs,n_meths))
            MeAE_A = np.empty((n_runs,n_meths))
            MeAE_B = np.empty((n_runs,n_meths))
